...
try:
    ...
    print(f"init_db::Error: {e}", file=open("errors.logs.txt", "a"))
        ...
        if "queries" in response.url:
            ...
            for results in json_data["results"]:
                try:
                    ...
                    print(f"handle_response::inserting_queries::Error: {e}", file=open(
                        "errors.logs.txt", "a"))
                    continue
...
async def get_page_html_data(base_url: str, current_page: Page, context: BrowserContext):
    ...
    while retries < max_retries:
        try:
            ...
            print(f"get_page_html_data::Error: {e}", file=open(
                "errors.logs.txt", "a"))
            traceback.print_exc()
            break
    if retries == max_retries:
...
async def crawl_website(page: Page, url: str, links: list[str], depth: int, browser_context: BrowserContext):
    ...
    for link in links:
        ...
        for locator in links_to_visit:
            try:
                ...
                print(
                    f"crawl_website::Timeout error while getting attribute {locator}", file=open("errors.logs.txt", "a"))
                continue
            except Exception as e:
...
async def main():
    async with async_playwright() as playwright:
        try:
            ...
            print(f"main::Error: {e}", file=open("errors.logs.txt", "a"))
